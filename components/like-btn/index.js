import React, {
  Component,
  StyleSheet,
  Text,
  Image,
  TouchableOpacity
} from 'react-native';

import {primary} from '../variables'

const base64Icon = 'data:image/png;base64,';

const LikeBtn = ({active, likes, color, onPress}) => (
  <TouchableOpacity onPress={onPress} style={styles.row}>
    {active ? <Image style={{width: 15, height: 15, tintColor: color}} source={{uri: base64Icon}}/> : <Image style={{width: 15, height: 15,tintColor: '#eee'}} source={{uri: base64Icon}}/>}
    <Text style={[styles.text, {color: color}]}>{likes} likes</Text>
  </TouchableOpacity>
);

const styles = StyleSheet.create({
  row: {
    flexDirection: 'row',
    alignItems: 'center',
  },
  text: {
    marginLeft: 5,
    fontWeight: '600'
  }
});
LikeBtn.defaultProps = {
  active: false,
  likes: 0,
  color: primary,
}
LikeBtn.propTypes = {
  active: React.PropTypes.bool,
  likes: React.PropTypes.number,
  color: React.PropTypes.string,
  onPress: React.PropTypes.func,
}
export default LikeBtn;
